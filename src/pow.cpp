...
{
    /* current difficulty formula, AILE - DarkGravity v3, written by Evan Duffield - evan@dashpay.io */
    ...
    if (pindexLast->nHeight >= Params().LAST_POW_BLOCK()) {
        ...
        uint256 bnNew;
        if (pindexLast->nHeight >= Params().LAST_POW_BLOCK() && pindexLast->nHeight < Params().LAST_POW_BLOCK() + 100) {
            bnNew = bnTargetLimit;
        } else {
