...
{
    // return if URI is not valid or is no AILE: URI
...
{
    // Convert aile:// to aile:
    //
    //    Cannot handle this later, because aile:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open aile.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "AILE.lnk";
}
...
{
    return GetAutostartDir() / "aile.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a aile.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=AILE\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the aile app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add aile app to startup item list
