...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        libzerocoin::CoinSpend zcspend = wtx.HasZerocoinPublicSpendInputs() ? ZAILEModule::parseCoinSpend(wtx.vin[0]) : TxInToZerocoinSpend(wtx.vin[0]);
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zaileTracker->HasMintTx(hash))) {
            //zAILE stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZAILE;
...
bool IsZAILEType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZAILE:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZAILE || type == TransactionRecord::MNReward) {
