...
#include "ui_zailecontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zaileTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zaileTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zAILE.";
            else if (!mint.isSeedCorrect)
                strReason = "The zAILE seed used to mint this zAILE is not the same as currently hold in the wallet";
            else
