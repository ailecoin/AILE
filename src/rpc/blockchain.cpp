...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zaileObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zaileObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zaileObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zAILEsupply", zaileObj));
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZAILE()) ? "Not available" : std::to_string(
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "  \"zAILEsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zAILE denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zAILE denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zAILE denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zAILE denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zAILE denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zAILE denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zAILE denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zAILE denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zAILE denominations\n"
            "  }\n"
            "}\n"
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in aile\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of aile addresses\n"
            "     \"aileaddress\"   	 	(string) aile address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZAILE_FAILED_ACCUMULATOR_INITIALIZATION);
