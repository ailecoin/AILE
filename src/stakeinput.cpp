...
// The zAILE block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zAILE is a hash of the serial
...
{
    //Create an output returning the zAILE that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZAILEOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zAILE output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zAILE", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZAILEOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zAILE output", __func__);
...
{
    CzAILETracker* zaileTracker = pwallet->zaileTracker.get();
    CMintMeta meta;
    if (!zaileTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zaileTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a AILE stake is the outpoint
