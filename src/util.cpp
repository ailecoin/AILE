...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "aile" is a composite category enabling all AILE-related debug output
            if (ptrCategory->count(std::string("aile"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\AILE
// Windows >= Vista: C:\Users\Username\AppData\Roaming\AILE
// Mac: ~/Library/Application Support/AILE
// Unix: ~/.aile
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AILE";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "aile.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty aile.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override aile.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ailed.pid"));
