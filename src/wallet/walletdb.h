...
// Copyright (c) 2016-2019 The AILE developers
...
public:
    ...
    bool WriteZAILESeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZAILESeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZAILESeed_deprecated(uint256& seed);
    bool EraseZAILESeed();
    bool EraseZAILESeed_deprecated();

    bool WriteZAILECount(const uint32_t& nCount);
    bool ReadZAILECount(uint32_t& nCount);
