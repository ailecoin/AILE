...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call AileCoin with fAggressive=true to
    // get as much data as possible.
    // Rewrite ailecoind data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> ailecoindData;
    bool allOK = dbenv.AileCoin(newFilename, true, ailecoindData);
    if (ailecoindData.empty()) {
        LogPrintf("AileCoin(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("AileCoin(aggressive) found %u records\n", ailecoindData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, ailecoindData) {
        if (fOnlyKeys) {
