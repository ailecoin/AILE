...
// Copyright (c) 2019-2020 The AILE developersf
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zaile/deterministicmint.h>
#include "zailetracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzAILETracker::CzAILETracker(std::string strWalletFile)
{
...
CzAILETracker::~CzAILETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzAILETracker::Init()
{
...
bool CzAILETracker::Archive(CMintMeta& meta)
{
...
bool CzAILETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzAILETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzAILETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzAILETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzAILETracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzAILETracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzAILETracker::GetSerialHashes()
{
...
CAmount CzAILETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzAILETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzAILETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzAILETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzAILETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzAILETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzAILETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzAILETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzAILETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzAILETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzAILETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzAILEWallet* zAILEWallet)
{
    bool iszAILEWalletInitialized = (NULL != zAILEWallet);
    CMintMeta meta;
    ...
    if (! iszAILEWalletInitialized)
        zAILEWallet = new CzAILEWallet(strWalletFile);
    meta.isSeedCorrect = zAILEWallet->CheckSeed(dMint);
    if (! iszAILEWalletInitialized)
        delete zAILEWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzAILETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAILETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzAILETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzAILETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzAILETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzAILETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzAILEWallet* zAILEWallet = new CzAILEWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zAILEWallet);
        }
        delete zAILEWallet;
        LogPrint("zero", "%s: added %d dzaile from DB\n", __func__, listDeterministicDB.size());
    }
