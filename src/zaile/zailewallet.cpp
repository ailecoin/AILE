...
CzAILEWallet::CzAILEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zaile seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZAILESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAILESeed_deprecated()) {
                    LogPrintf("%s: Updated zAILE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zaile seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zaile wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzAILEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAILECount(nCountLastUsed);
    else if (!walletdb.ReadZAILECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAILEWallet::Lock()
{
    seedMaster = 0;
}

void CzAILEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAILEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAILE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAILEWallet::LoadMintPoolFromDB()
{
...
void CzAILEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAILEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAILEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zaileTracker->HasPubcoinHash(pMint.first)) {
...
bool CzAILEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZAILE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zaileTracker which also adds to database
    pwalletMain->zaileTracker->Add(dMint, true);
...
void CzAILEWallet::SeedToZAILE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzAILEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAILEWallet::GenerateDeterministicZAILE(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAILEWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAILEWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzAILEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
